...
namespace BizTalk.Adapter.SBQueue
{
    ...
    public void SubmitBatch()
    {
        ...
        try
        {
            ...
            if (SBQueueReceiver.clients.TryGetValue(clientKey, out client) == false)
            {
                ServiceBusConnectionStringBuilder conn = new ServiceBusConnectionStringBuilder(connectionString);
                client = QueueClient.CreateFromConnectionString(connectionString, properties.Queue, ReceiveMode.PeekLock);
                
               
                SBQueueReceiver.clients.TryAdd(clientKey, client);
            ...
            lock (client)
            {
                ...
                if (properties.UseSession == true)
                {
                    ...
                    {
                        GetAllMessagesWithSession(client);
                    }
                    else
                    {
                        GetSessionMessages(client);
                    }
